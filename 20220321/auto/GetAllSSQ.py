...
def getSsqSum(this_year,this_month,product_id,store_code):
    ...
    with conn.cursor() as cursor:
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            insertSsqSum(id,store_code,product_id,this_year,this_month,str(myresult[0]).split('.')[0])#id,store_code,product_id,year,this_month,value
...
def insertSsqSum(id,store_code,product_id,year,this_month,value):
    ...
    with conn.cursor() as cursor:
        ...
        try:
            ...
            sql="insert into mssq (id,store_code,product_id,year,"+str(this_month_arr[this_month])+") values ("+str(id)+","+str(store_code)+","+str(product_id)+","+str(year)+","+str(value)+")"
            # print("sql : insert pp(id,international_barcode,product_name,spec,broken_number,suggested_price,purchase_price,tax,normal_gift,average_price,first_stage,second_stage,first_stage_average_price,second_stage_average_price,gp_after_new_avg_price_of_5_boxes,old_declared_purchase_price,old_way_to_declare_discounts,avg_price_of_old_declaration,more_than_five_boxes) values('"+str(id)+"','"+str(round(international_barcode))+"','"+str(spec)+"','"+str(broken_number)+"','"+str(suggested_price)+"','"+str(purchase_price)+"','"+str(tax)+"','"+str(normal_gift)+"','"+str(average_price)+"','"+str(first_stage)+"','"+str(second_stage)+"','"+str(first_stage_average_price)+"','"+str(second_stage_average_price)+"','"+str(gp_after_new_avg_price_of_5_boxes)+"','"+str(old_declared_purchase_price)+"','"+str(old_way_to_declare_discounts)+"','"+str(avg_price_of_old_declaration)+"','"+str(more_than_five_boxes)+"')")
            print("sql : "+str(sql))
            cursor.execute(sql)
        except Exception as e:
            print(e)
            sql="UPDATE mssq SET id = "+str(id)+", store_code = "+str(store_code)+", product_id = "+str(product_id)+", year = "+str(year)+", "+str(this_month_arr[this_month])+" = "+str(value)+" WHERE id = "+str(id)+" "#"UPDATE runoob_tbl SET runoob_title='学习 C++' WHERE runoob_id=3"
            print("sql : "+str(sql))
...
def allssq():
    ...
    #月份補0 report_month='%02d' % c.getMonth(weeks=4)

    start=3

    print(report_month)
    print(report_year)
    print(str(int(report_year)-1))
    print("今年")
    #time.sleep(20)
    for i in range(1,start+1,1):#1 2
    ...
    print(report_year)
    for i in range(start+1,13,1):#4 5 6 7 8 9 10 11 12
