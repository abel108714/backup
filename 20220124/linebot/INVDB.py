...
def getDataArr(product_msg):
    ...
    if product_msg.isdigit() == True:
        ...
        with conn.cursor() as cursor:
            # sql="select product_id as 品號,product_name as 品名,packing as 包裝方式,inv_qty as 庫存數量,packing_qty as 包裝數量,exp as 有效期限 from inv where product_name like '%"+str(product_msg)+"%'" #cast(inv_qty as signed)
            ...
            sql="SELECT product_id as 品號,product_name as 品名,packing as 包裝方式,inv_qty as 庫存數量,packing_qty as 包裝數量,exp as 有效期限 FROM inv WHERE inv_qty > 0 and product_name like '%"+str(product_msg)+"%'"+"and STR_TO_DATE(exp, '%Y-%m-%d') IS NOT NULL GROUP BY product_id, product_name ORDER BY DATE_FORMAT(exp, '%Y-%m-%d') ASC"
            print(sql)
            cursor.execute(sql)
            myresult = cursor.fetchall()
            # print(myresult)
