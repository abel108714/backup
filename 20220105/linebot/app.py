...
def job2():
    ...
    if date.strftime("%H:%M") == "16:29":#"17:55":#自動發網通報表
...
def handle_message(event):#
    ...
    # salesman_id="Uee6224531167e863e3c08504055d6ed2"#U8ec81edfb39499a39625be8c3e335ce0
    # guest_id="123"
    # admin_user_id="Uee6224531167e863e3c08504055d6ed2"
    salesman_id=""
    dict = {}
    ...
    if msg.isdigit() and (user_id == admin_user_id or user_id == salesman_id):
        print("數字msg")
        print("msg : " + msg)
        print("user_id : " + user_id)
        path='C:\\linebot\\msg\\'
    ...
    if msg.isdigit() == False and (user_id == admin_user_id or user_id == salesman_id) and (msg != "[庫存查詢]" and msg != "[進價與促銷]" and msg != "[庫存及效期]" and msg != "[run]" and msg !="[補發門市報表]" and msg !="[補發網通報表]" and msg !="[網通報表測試]" and msg !="[下一頁]"):
        print("文字msg")
        print("msg : " + msg)
        print("user_id : " + user_id)
        path='C:\\linebot\\msg\\'
    ...
    print(user_id)
    print(salesman_id)
    if msg != "在?" and (user_id == admin_user_id or user_id == salesman_id):
        print("RichMenu準備刪除")
        if isRichMenu(line_bot_api) == True and (user_id == admin_user_id or user_id == salesman_id):#若有RichMenu，則刪除，準備新增
            delRichMenu(line_bot_api)
            print("RichMenu，已刪除，準備新增")
            print(isRichMenu(line_bot_api))

        if isRichMenu(line_bot_api) == False and (user_id == admin_user_id or user_id == salesman_id):#若無RichMenu，則新增
            print("RichMenu開始新增")
            try:
                # 嘗試執行一些程式碼
                # headers = {"Authorization":"Bearer 8GR7MNmmUmntbkHzoZK4YszKb4hOktoAxy+Bzp429ktQgNCMPP+KnKGwxeotxQY2ti1GrWqcgez0P0egY1iIA1BXx8/9WTMdPGnTaK/rUMD3wSn6eHzuU6cg/aj4BPx/phaQusTMdx3bYwh2iQfStgdB04t89/1O/w1cDnyilFU=","Content-Type":"application/json"}
    ...
    if '[補發門市報表]' in msg and user_id == admin_user_id :
        ...
        #     ]
        # )
    if '[sss]' in msg and (user_id == admin_user_id or user_id == salesman_id):
        # YahooWebCrawler(2330)
        print('123')
    ...
    if '[進價與促銷]' in msg  and (user_id == admin_user_id or user_id == salesman_id):
        print(msg)
        # print("123")
        ...
        #check pp
            ...
            print("例外")
        closeThread()
    if ('[庫存查詢]' in msg or '[庫存及效期]' in msg ) and (user_id == admin_user_id or user_id == salesman_id):
        print(msg)
        # print("123")
        print("user_id : "+str(user_id))
        path='C:\\linebot\\msg\\'
        ...
        if len(DataArrOfINVDB01002)==0:
            print("成品倉缺貨或無此品項，或請減少關鍵字，再重新查詢")
            MsgStr=MsgStr+"成品倉缺貨或無此品項，或請減少關鍵字，再重新查詢"
        else:
            for i in range(len(DataArrOfINVDB01002)):
                # print("i = "+str(i))
                # print("品號 : "+str(DataArrOfINVDB01002[i][0]))
                # print("品名 : "+str(DataArrOfINVDB01002[i][1]))
                print(" : "+str(DataArrOfINVDB01002[i][0]))
                print(" : "+str(DataArrOfINVDB01002[i][1]))
                print(" : "+str(DataArrOfINVDB01002[i][2]))
                print(" : "+str(DataArrOfINVDB01002[i][3]))
                print(" : "+str(DataArrOfINVDB01002[i][4]))
                print(" : "+str(DataArrOfINVDB01002[i][5]))
                print(" : "+str(DataArrOfINVDB01002[i][6]))
                print(" : "+str(DataArrOfINVDB01002[i][7]))
                print(" : "+str(DataArrOfINVDB01002[i][8]))
    ...
    if '[網通報表測試]' in msg and user_id == admin_user_id :
        ...
        try:
            ...
            print("linebot無回應: "+str(e))
        closeThread()
    if '[下一頁]' in msg and (user_id == admin_user_id or user_id == salesman_id) :
        print("[下一頁]")
