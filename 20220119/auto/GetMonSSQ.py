...
def getYear(string):
    RemoveSemicolonStr = string.split(':')
    print(RemoveSemicolonStr[0])
    print(RemoveSemicolonStr[1])
    print(RemoveSemicolonStr[2])
    RemoveWhitespaceStr = RemoveSemicolonStr[2].split(' ')
    print(RemoveWhitespaceStr[0])
    print(RemoveWhitespaceStr[1].split('/'))
    # print(RemoveWhitespaceStr[2])
    return RemoveWhitespaceStr[0]
# def checkString(string):
#     pos=isQuotationMarksExist(string)
#     if pos!= -1:
...
def insertSsqData(id,store_code,product_id,product_name,year,january,february,march,april,may,june,july,august,september,october,november,december):
# CREATE Table ssq(
...
#insert pp(id,international_barcode,spec,broken_number,suggested_price,purchase_price,tax,normal_gift,average_price,first_stage,second_stage,first_stage_average_price,second_stage_average_price,more_than_five_boxes)values('11702023','4719859749386','30g*25入/盒*6','260','600','260','應','(2入800)11組贈1組','238.33','250(12盒以上)','22+2盒','250.00','238.33','230.00');
    ...
    with conn.cursor() as cursor:
        sql=""
        # if not product_id is None:#=="11503010":
        #     print(product_id)
        #     print(product_id=="11704121")
        #     time.sleep(10)
        #     if product_id=="11704121":
        #         print("123")
        # if product_id=="11704121":#"11503010":
        #     print("ok")
        #     time.sleep(10)
        # print("insertPPData123")
        try:
            # print("insertPPData456")
                ...
                # ) \
            # if str(product_id)==str("11503010"):
            #     time.sleep(10)
            if id is None:
                id=""
            ...
            # print("7777777")
            sql="insert ssq(\
                    id,store_code,product_id,product_name,year, \
                    january,february,march,april,may,june, \
                    july,august,september,october,november,december \
                ) \
                values(\
                    '"+str(id)+"','"+str(store_code)+"','"+str(product_id)+"','"+checkString(str(product_name))+"','"+str(year)+"','"+str( \
                    january)+"','"+str(february)+"','"+str(march)+"','"+str(april)+"','"+str(may)+"','"+str(june)+"','"+str( \
                    july)+"','"+str(august)+"','"+str(september)+"','"+str(october)+"','"+str(november)+"','"+str(december)+"' \
                )"
...
def main(argv):
    ...
    for k in range(1,res+1):
        for j in range(4,max_row+1):
            ...
            if j==5:#第三行為空表無資料，跳過
                continue         
            if str(ws.range(j,1).value) == str(None):#第三行為空表無資料，跳過
                continue
            if str(ws.range(j,1).value) == str("專櫃合計："):#到專櫃合計結束
                break
            # if (j==5)==True:#第三行為空表無資料，跳過
            #     continue
            ...
            RowData = [i for i in ws.range(str(j)+':'+str(j)).value if j <= max_row]
            if j==4:
                # store_code=getStoreCode(RowData[8])
                print(RowData[7])
                year=getYear(RowData[7])
            # if k==188 and j==23:
            #     time.sleep(4)
            ...
            #     if isSubstringExist(str(RowData[1]),"銷貨小計")==False and isSubstringExist(str(RowData[1]),"退貨小計")==False:
                    ...
                    # print("res = "+str(res))
            # year="2020"
            # id=str(RowData[0])+str(store_code)+str(year)
            # print("id="+str(id))
            # print("店號="+str(store_code))
            print("RowData[0]品號="+str(RowData[0]))
            print("RowData[1]品名="+str(RowData[1]))
            # print("RowData[2]客戶代號="+str(RowData[2]))
            # if str(RowData[0])==str("11503010"):
            #     time.sleep(10)
            print("year="+str(year))
            print("RowData[3]一月="+str(RowData[3]))
            print("RowData[4]二月="+str(RowData[4]))
            print("RowData[5]三月="+str(RowData[5]))
            print("RowData[6]四月="+str(RowData[6]))
            print("RowData[7]五月="+str(RowData[7]))
            print("RowData[8]六月="+str(RowData[8]))
            print("RowData[9]七月="+str(RowData[9]))
            print("RowData[10]八月="+str(RowData[10]))
            print("RowData[11]九月="+str(RowData[11]))
            print("RowData[12]十月="+str(RowData[12]))
            print("RowData[13]十一月="+str(RowData[13]))
            print("RowData[14]十二月="+str(RowData[14]))
            # time.sleep(10)
            # if j>4:
            #     insertSsqData(id,store_code,RowData[0],RowData[1],year,RowData[3],RowData[4],RowData[5],RowData[6],RowData[7],RowData[8],RowData[9],RowData[10],RowData[11],RowData[12],RowData[13],RowData[14])
            # print("RowData[15]一月="+str(RowData[15]))
            # print("RowData[16]一月="+str(RowData[16]))
            # print("RowData[17]一月="+str(RowData[17]))
            # print("RowData[18]一月="+str(RowData[18]))
            # print("RowData[19]一月="+str(RowData[19]))
            # print("RowData[20]一月="+str(RowData[20]))#library_code		
            # print("RowData[21]一月="+str(RowData[21]))#library_code			
        ...
        # factory_code)+"','"+str(library_code)+"'\
                    ...
                    # insertSsqData(RowData[0],RowData[1],RowData[2],RowData[3],RowData[4],RowData[5],RowData[6],RowData[7],RowData[11],RowData[12],RowData[13],RowData[14],RowData[15],RowData[16],RowData[17],RowData[18],RowData[19],RowData[20])
        # k=k+1
        print(k) 
        print(res) 
        # if k == 27:
...
if __name__ == "__main__":
    ...
    #main("S:\網通部\◎資訊\年月均銷量表\POSR0720210831000507202108310001.XLS")#2020
    #main("S:\網通部\◎資訊\年月均銷量表\COPR2320210824000056202108240001.XLS")
    # main(sys.argv)
    #main("S:\網通部\◎資訊\年月均銷量表\POSR0720210804000130202108040001.XLSX")
    main("C:\My Documents\POSR4020210916000061202109160001A.XLS")
    # main("S:\網通部\◎資訊\年月均銷量表\POSR07202108040001302021080400012.XLSX")
