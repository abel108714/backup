...
def getDataArrOfProm(product_msg):
    ...
    with conn.cursor() as cursor:
        ...
        sql="select pp.id as 品號,pp.international_barcode as 國際條碼,pp.product_name as 品名,pp.spec as 規格,pp.broken_number as 折數,pp.suggested_price as 建議售價,pp.purchase_price as 進價,pp.tax as 免應稅,pp.normal_gift as 常態搭贈,pp.average_price as 均價,pp.first_stage as 第一階,pp.second_stage as 第二階,pp.first_stage_average_price as 第一階均價,pp.second_stage_average_price as 第二階均價,pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利,pp.old_declared_purchase_price as 舊申報進價,pp.old_way_to_declare_discounts as 舊申報優惠方式,pp.avg_price_of_old_declaration as 舊申報均價,pp.more_than_five_boxes as 五箱以上, prom.special_offer as 特價, prom.special_purchase_price as 特進價, prom.note as 備註  from pp join prom on prom.id = pp.id where pp.product_name like '%"+str(product_msg)+"%'"
        print(sql)
        cursor.execute(sql)
        myresult = cursor.fetchall()
        conn.commit()
