...
def insertPPData(id,international_barcode,product_name,spec,expected_price_increase_date,broken_number,suggested_price,purchase_price,tax,normal_gift,average_price,first_stage,second_stage,first_stage_average_price,second_stage_average_price,gp_after_new_avg_price_of_5_boxes,old_declared_purchase_price,old_way_to_declare_discounts,avg_price_of_old_declaration,more_than_five_boxes):
...
#insert pp(id,international_barcode,spec,broken_number,suggested_price,purchase_price,tax,normal_gift,average_price,first_stage,second_stage,first_stage_average_price,second_stage_average_price,more_than_five_boxes)values('11702023','4719859749386','30g*25入/盒*6','260','600','260','應','(2入800)11組贈1組','238.33','250(12盒以上)','22+2盒','250.00','238.33','230.00');
    ...
    with conn.cursor() as cursor:
        ...
        try:
            ...
            if spec is None:
                spec=""
            print("expected_price_increase_date = "+str(expected_price_increase_date))
            if expected_price_increase_date is None:
                expected_price_increase_date=""
            ...
            # avg_price_of_old_declaration,more_than_five_boxes) values('"+str(id)+"','"+str(international_barcode)+"','"+str(spec)+"','"+str(broken_number)+"','"+str(suggested_price)+"','"+str(purchase_price)+"','"+str(tax)+"','"+str(normal_gift)+"','"+str(average_price)+"','"+str(first_stage,second_stage)+"','"+str(first_stage_average_price)+"','"+str(second_stage_average_price)+"','"+str(gp_after_new_avg_price_of_5_boxes)+"','"+str(old_declared_purchase_price)+"','"+str(old_way_to_declare_discounts)+"','"+str(avg_price_of_old_declaration)+"','"+str(more_than_five_boxes)+"')"
            print("insertPPData")
            print(international_barcode)
            print(round(international_barcode))

            sql="insert pp(\
                    id,international_barcode,product_name,spec,expected_price_increase_date,broken_number,suggested_price,\
                ...
                values('\
                    "+str(id)+"','"+str(round(international_barcode))+"','"+str(checkString(product_name))+"','"+str(spec)+"','"+str(expected_price_increase_date)+"','"+str(broken_number)+"','"+str(suggested_price)+"',\
            ...
            print(e)
            print("錯誤")
            sql2 = "DELETE FROM pp where id = "+str(int(id))
            print("sql2 = "+str(sql2))
            cursor.execute(sql2)
            cursor.execute(sql)
...
def main(argv):
    ...
    print("j")
    # print(ws.range('1:1').value[0]) 
    # print(ws.range('2:2').value[1]) 
    # print(ws.range('3:3').value[2])
    start_row = 6
    for j in range(start_row,max_row+1):
        ...
        #         print("max_row = "+str(max_row))   
        #         #print(ws)  
        #         #print(ws.range(str(j)+':'+str(j)).value) 
        #         # print(ws.range(str(j)+':'+str(j)).value[0]) 
        #         # print(ws.range(str(j)+':'+str(j)).value[1]) 
        #         # print(ws.range(str(j)+':'+str(j)).value[2]) 
        #         print(ws.range('6:6').value[0]) 
        #         print(ws.range('6:6').value[1]) 
        #         print(ws.range('6:6').value[2]) 
        ...
        #         time.sleep(10)
        #         print("123")
        #     # for i in ws.range(str(j)+':'+str(j)).value:
        #     #     print(i)
        print("aaaa")
        time.sleep(2)
        print("bbbb")
        ...
        #     print("ccccc")
        #     #print(ws.range(str(j)+':'+str(j)).value) 
        #     #RowData = [i for i in ws.range(str(j)+':'+str(j)).value]
        #     #RowData = [i for i in ws.range('A'+str(j)+':'+'V'+str(j)).value]
        #     k=0
            
        #     for i in ws.range('A'+str(j)+':'+'V'+str(j)).value:
        #         print("j = "+str(j))
        #         print("k = "+str(k))
        #         RowData = i
        #         #RowData[0] = RowData[0].split('.')[0]
        #         #print("RowData[0] = "+str(RowData[0]))
        #         #RowData[0]=str(round(RowData[0]))
        #         #print("RowData[0] = "+str(RowData[0]))
        #         print("RowData = "+str(RowData))
        #         AA[k] = RowData
        #         print("AA[k] = "+str(AA[k]))
        #         k=k+1
        # print(ws.range('B'+str(j)).value) 
        # if ws.range('B'+str(j)).value == None:
        #     print(yyyyy)
        RowData = [
            i for i in ws.range(str(j)+':'+str(j)).value if j <= max_row #and ws.range('E'+str(j)).value == None
        ...
        print("品號:" + str(RowData[0]))
        
        print("國際條碼:" + str(RowData[1]))
        print(":" + str(RowData[2]))
        print(":" + str(RowData[3]))
        print(":" + str(RowData[4]))
        print(":" + str(RowData[5]))
        ...
        print("保存期限:" + str(RowData[11]))#保存期限
        print(":" + str(RowData[12]))
        print(":" + str(RowData[13]))
        print(":" + str(RowData[14]))
        print(":" + str(RowData[15]))
        if RowData[16] == None:
            RowData[16] = ""
        print(":" + RowData[16])
        if RowData[17] == None:
        ...
        insertPPData(RowData[0],RowData[1],RowData[2],RowData[3],RowData[4],RowData[5],RowData[6],RowData[7],RowData[8],RowData[10],RowData[12],RowData[13],RowData[14],RowData[15],RowData[16],RowData[17],RowData[18],RowData[19],RowData[20],RowData[21])
...
if __name__ == "__main__":
    ...
    #main(r"C:\\Users\\udev77\\Desktop\\test.xlsx")
