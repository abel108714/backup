    ...
    try:
        ...
        if product_msg.isdigit() == True:
            with conn.cursor() as cursor:
                ...
                sql="select product_id as 品號, product_name as 品名, spec as 規格, factory_code as 廠別代號, factory_name as 廠別名稱, inv_qty as 庫存數量, unit as 單位, small_unit as 小單位, storage_spaces as 儲位, quantity_at_the_beginning_of_the_month as 月初數量, the_most_recent_storage_date as 最近入庫日, the_most_recent_out_of_the_warehouse_day as 最近出庫日, last_count_date as 上次盤點日, safety_stock as 安全存量, replenishment_point as 補貨點, economic_batch as 經濟批量, standard_inventory as 標準存貨量, standard_turnover_rate as 標準週轉率, inventory_packaging_quantity as 庫存包裝數量, package_quantity_at_the_beginning_of_the_month as 月初包裝數量 from inv01002 where product_id='"+str(product_msg)+"'"+"limit 1"
                print(sql)
                cursor.execute(sql)
            ...
            with conn.cursor() as cursor:
                ...
                sql="select product_id as 品號, product_name as 品名, spec as 規格, factory_code as 廠別代號, \
                    factory_name as 廠別名稱, inv_qty as 庫存數量, unit as 單位, small_unit as 小單位, \
                    storage_spaces as 儲位, quantity_at_the_beginning_of_the_month as 月初數量, \
                    the_most_recent_storage_date as 最近入庫日, the_most_recent_out_of_the_warehouse_day as 最近出庫日, \
                    last_count_date as 上次盤點日, safety_stock as 安全存量, replenishment_point as 補貨點, \
                    economic_batch as 經濟批量, standard_inventory as 標準存貨量, standard_turnover_rate as 標準週轉率, \
                    inventory_packaging_quantity as 庫存包裝數量, package_quantity_at_the_beginning_of_the_month as 月初包裝數量 \
                    from inv01002 where inv_qty > 0 and product_name like '%"+str(product_msg)+"%' limit 10"
                print(sql)
                cursor.execute(sql)
                myresult = cursor.fetchall()
                # print(myresult)
