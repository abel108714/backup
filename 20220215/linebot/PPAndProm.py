...
#             # 儲存變更
#             conn.commit()
# def insertPromData(id,special_offer,special_purchase_price,note):
#     conn = pymysql.connect(**setDB())
#     # 建立Cursor物件
#     with conn.cursor() as cursor:
...
#             sql="insert prom(id,special_offer,special_purchase_price,note) values('"+str(id)+"','"+str(special_offer)+"','"+str(special_purchase_price)+"','"+str(note)+"')"#+"','"+str(first_stage_average_price)+"','"+str(second_stage_average_price)
#             print("sql : "+str(sql))
...
# def insertPPData(id,international_barcode,spec,broken_number,suggested_price,purchase_price,tax,normal_gift,average_price,first_stage,second_stage,first_stage_average_price,second_stage_average_price,gp_after_new_avg_price_of_5_boxes,old_declared_purchase_price,old_way_to_declare_discounts,avg_price_of_old_declaration,more_than_five_boxes):
...
#             print("insertPPData")
#             sql="insert pp(\
#                     id,international_barcode,spec,broken_number,suggested_price,\
...
#                 ) \
#                 values('\
#                     "+str(id)+"','"+str(round(international_barcode))+"','"+str(spec)+"','"+str(broken_number)+"','"+str(suggested_price)+"',\
...
#         # 儲存變更
#         conn.commit()
def getDataArrOfPPAndProm(product_msg):
    print("getDataArrOfPPAndProm 1")
    conn = pymysql.connect(**setDB())
    print(conn)
    print("getDataArrOfPPAndProm ok")
    if product_msg.isdigit() == True:
        conn = pymysql.connect(**setDB())
        print(conn)
        print("product_msg True")
        with conn.cursor() as cursor:
            ...
            sql="select \
                pp.id as 品號,\
                pp.international_barcode as 國際條碼,\
                inv.product_name as 品名, \
                pp.spec as 規格, \
                pp.broken_number as 折數, \
                pp.suggested_price as 建議售價, \
                pp.purchase_price as 進價, \
                pp.tax as 免應稅, \
                pp.normal_gift as 常態搭贈, \
                pp.average_price as 均價, \
                pp.first_stage as 第一階, \
                pp.second_stage as 第二階, \
                pp.first_stage_average_price as 第一階均價, \
                pp.second_stage_average_price as 第二階均價, \
                pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利, \
                pp.old_declared_purchase_price as 舊申報進價, \
                pp.old_way_to_declare_discounts as 舊申報優惠方式, \
                pp.avg_price_of_old_declaration as 舊申報均價, \
                pp.more_than_five_boxes as 五箱以上, \
                prom.special_offer as 特價, \
                prom.special_purchase_price as 特進價, \
                prom.note as 備註 \
            from inv  \
            join pp on pp.id = inv.product_id  \
            join prom on prom.id = inv.product_id  \
            where  \
                inv.product_id = '"+str(product_msg)+"'"
            print(sql)
            cursor.execute(sql)
        ...
        try:
            with conn.cursor() as cursor:
                ...
                join prom on prom.id = inv.product_id  \
                where  \
                    inv.product_name like '%"+str(product_msg)+"%'"
                # sql="select pp.id as 品號,pp.international_barcode as 國際條碼,inv.product_name as 品名, pp.spec as 規格, pp.broken_number as 折數, pp.suggested_price as 建議售價, pp.purchase_price as 進價, pp.tax as 免應稅, pp.normal_gift as 常態搭贈, pp.average_price as 均價, pp.first_stage as 第一階, pp.second_stage as 第二階, pp.first_stage_average_price as 第一階均價, pp.second_stage_average_price as 第二階均價, pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利, pp.old_declared_purchase_price as 舊申報進價, pp.old_way_to_declare_discounts as 舊申報優惠方式, pp.avg_price_of_old_declaration as 舊申報均價, pp.more_than_five_boxes as 五箱以上, prom.special_offer as 特價, prom.special_purchase_price as 特進價, prom.note as 備註 from inv join pp on pp.id = inv.product_id join prom on prom.id = inv.product_id where inv.product_name like '%"+str(product_msg)+"%'"
                print(sql)
                cursor.execute(sql)
                myresult = cursor.fetchall()
                ...
                #     # print("data="+str(data))
                #     i=i+1
                print(str(myresult) == str("()"))
                if str(myresult) == str("()"):
                    with conn.cursor() as cursor:
                        print("88888888888888888")


# select * from pp where product_name like '%%%杏%仁%飲%%%';
# select * from prom where product_name like '%%杏%仁%%';               

# 沒prom
# select pp.id as 品號,pp.international_barcode as 國際條碼,inv.product_name as 品名,pp.spec as 規格,pp.broken_number as 折數,pp.suggested_price as 建議售價,pp.purchase_price as 進價,pp.tax as 免應稅,pp.normal_gift as 常態搭贈,pp.average_price as 均價,pp.first_stage as 第一階,pp.second_stage as 第二階,pp.first_stage_average_price as 第一階均價,pp.second_stage_average_price as 第二階均價,pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利,pp.old_declared_purchase_price as 舊申報進價,pp.old_way_to_declare_discounts as 舊申報優惠方式,pp.avg_price_of_old_declaration as 舊申報均價,pp.more_than_five_boxes as 五箱以上 from inv join pp on pp.id = inv.product_id where inv.product_name like '%%%杏%仁%飲%%%'

# 沒pp
# select prom.id as 品號,prom.special_offer as 特價,prom.special_purchase_price as 特進價,prom.note as 備註 from inv join pp on pp.id = inv.product_id where inv.product_name like '%%%杏%仁%飲%%%'
                        ...
                        #     prom.note as 備註 \
                        # from prom  \
                        # join pp on pp.id = prom.id  \
                        # where  \
                        #     pp.product_name like '%"+str(product_msg)+"%'"
                        # sql="select pp.id as 品號,pp.international_barcode as 國際條碼,inv.product_name as 品名, pp.spec as 規格, pp.broken_number as 折數, pp.suggested_price as 建議售價, pp.purchase_price as 進價, pp.tax as 免應稅, pp.normal_gift as 常態搭贈, pp.average_price as 均價, pp.first_stage as 第一階, pp.second_stage as 第二階, pp.first_stage_average_price as 第一階均價, pp.second_stage_average_price as 第二階均價, pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利, pp.old_declared_purchase_price as 舊申報進價, pp.old_way_to_declare_discounts as 舊申報優惠方式, pp.avg_price_of_old_declaration as 舊申報均價, pp.more_than_five_boxes as 五箱以上, prom.special_offer as 特價, prom.special_purchase_price as 特進價, prom.note as 備註 from inv join pp on pp.id = inv.product_id join prom on prom.id = inv.product_id where inv.product_name like '%"+str(product_msg)+"%'"
                        # sql="select * from prom where product_name like '%"+str(product_msg)+"%'"
                        # print(sql)
                        # cursor.execute(sql)
                        # prom_myresult = cursor.fetchall()

                        # sql="select * from pp where product_name like '%"+str(product_msg)+"%'"
                        # print(sql)
                        # cursor.execute(sql)
                        # pp_myresult = cursor.fetchall()

                        # if str(prom_myresult) == str("()"):

                        # if str(pp_myresult) == str("()"):
...
def getDataArrOfProm(product_msg):
    conn = pymysql.connect(**setDB())
    print(product_msg)
    product_msg=addPerSignToSpacing(product_msg)
    print(product_msg)
    ...
    print(product_msg)
    with conn.cursor() as cursor:
        # sql="select pp.id as 品號,pp.international_barcode as 國際條碼,pp.product_name as 品名,pp.spec as 規格,pp.broken_number as 折數,pp.suggested_price as 建議售價,pp.purchase_price as 進價,pp.tax as 免應稅,pp.normal_gift as 常態搭贈,pp.average_price as 均價,pp.first_stage as 第一階,pp.second_stage as 第二階,pp.first_stage_average_price as 第一階均價,pp.second_stage_average_price as 第二階均價,pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利,pp.old_declared_purchase_price as 舊申報進價,pp.old_way_to_declare_discounts as 舊申報優惠方式,pp.avg_price_of_old_declaration as 舊申報均價,pp.more_than_five_boxes as 五箱以上, prom.special_offer as 特價, prom.special_purchase_price as 特進價, prom.note as 備註 from pp join prom on prom.product_name = pp.product_name where pp.product_name like '%"+str(product_msg)+"%'"
        sql="select pp.id as 品號,pp.international_barcode as 國際條碼,pp.product_name as 品名,pp.spec as 規格,pp.broken_number as 折數,pp.suggested_price as 建議售價,pp.purchase_price as 進價,pp.tax as 免應稅,pp.normal_gift as 常態搭贈,pp.average_price as 均價,pp.first_stage as 第一階,pp.second_stage as 第二階,pp.first_stage_average_price as 第一階均價,pp.second_stage_average_price as 第二階均價,pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利,pp.old_declared_purchase_price as 舊申報進價,pp.old_way_to_declare_discounts as 舊申報優惠方式,pp.avg_price_of_old_declaration as 舊申報均價,pp.more_than_five_boxes as 五箱以上, prom.special_offer as 特價, prom.special_purchase_price as 特進價, prom.note as 備註  from pp left join prom on prom.id = pp.id where pp.product_name like '%"+str(product_msg)+"%'"
        print(sql)
        cursor.execute(sql)
        myresult = cursor.fetchall()
        conn.commit()
...
def getDataArrOfPP(product_msg):
    conn = pymysql.connect(**setDB())
    print(product_msg)
    product_msg=addPerSignToSpacing(product_msg)
    print(product_msg)
    ...
    print(product_msg)
    with conn.cursor() as cursor:
        sql="select pp.id as 品號,pp.international_barcode as 國際條碼,pp.product_name as 品名,pp.spec as 規格,pp.broken_number as 折數,pp.suggested_price as 建議售價,pp.purchase_price as 進價,pp.tax as 免應稅,pp.normal_gift as 常態搭贈,pp.average_price as 均價,pp.first_stage as 第一階,pp.second_stage as 第二階,pp.first_stage_average_price as 第一階均價,pp.second_stage_average_price as 第二階均價,pp.gp_after_new_avg_price_of_5_boxes as 新5箱均價後毛利,pp.old_declared_purchase_price as 舊申報進價,pp.old_way_to_declare_discounts as 舊申報優惠方式,pp.avg_price_of_old_declaration as 舊申報均價,pp.more_than_five_boxes as 五箱以上 from pp where pp.product_name like '%"+str(product_msg)+"%'"
        print(sql)
        cursor.execute(sql)
        myresult = cursor.fetchall()
        conn.commit()
...
# def getDataArrOfPPAndProm(product_id):
#     conn = pymysql.connect(**setDB())
#     with conn.cursor() as cursor:
...
#             join prom on prom.id = inv.product_id  \
#             where  \
#                 inv.product_name like '"+str(product_id)+"'"
#         print(sql)
#         cursor.execute(sql)
#         myresult = cursor.fetchall()
#         # print(myresult)
...
def main(argv):
    ...
    for j in range(5,max_row+1):
        ...
        insertPPData(RowData[0],RowData[1],RowData[3],RowData[4],RowData[5],RowData[6],RowData[7],RowData[8],RowData[10],RowData[11],RowData[12],RowData[13],RowData[14],RowData[15],RowData[16],RowData[17],RowData[18],RowData[19])
...
def main2(argv):
    ...
    for j in range(5,max_row+1):
        ...
        insertPromData(RowData[1],RowData[4],RowData[5],RowData[6])
