...
def main(argv):
    ...
    for j in range(2,max_row+1):
        ...
        newDataAccess=FileDataAccess(0,'2','-','S:\開放公用區\★每日更新庫存水位警示表★\\小幫手庫存提醒訊息.txt')
        updateArr=['','']
        if old_product_id != product_id:
            ...
            if six_mos_warning != "None":
                ...
                six_mos_warning_title_match = "六個月賣不完"
                six_mos_warning_msg=six_mos_warning_msg+"\n"+product_id+" "+product_name#+" "+rep_notice+" "+six_mos_warning+" "+three_mos_warning
                six_mos_warning_updateArr = [six_mos_warning_msg]
            ...
            if  three_mos_warning != "None":
                ...
                # print(six_mos_warning)
                # print(three_mos_warning)
                three_mos_warning_title_match = "三個月賣不完"
                three_mos_warning_msg=three_mos_warning_msg+"\n"+product_id+" "+product_name#+" "+rep_notice+" "+six_mos_warning+" "+three_mos_warning
                three_mos_warning_updateArr = [three_mos_warning_msg]
                
    # title_match=rep_notice_title_match
    # updateArr=rep_notice_updateArr
    # newDataAccess.setData(title_match,updateArr)

    title_match=six_mos_warning_title_match
    updateArr=six_mos_warning_updateArr
    newDataAccess.setData(title_match,updateArr)

    title_match=three_mos_warning_title_match
    updateArr=three_mos_warning_updateArr
    newDataAccess.setData(title_match,updateArr)
...
if __name__ == "__main__":
    ...
    file_name = r"C:\Users\udev77\Desktop\★庫存水位-唯讀參閱不可修改★.xlsm"
    print(file_name)
