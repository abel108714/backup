...
def job2():
    ...
    elif date.strftime("%H:%M") == "16:58":#測試用時間
        print('case4')
        line_bot_api.push_message("U03f8575fac6ad12b612c6222ad37678e", OCPerformanceReport())
        sql="INSERT INTO users(checked) VALUES(%s) where UID=\"%s\""
        data=(tdy_date_str,str(event.source.user_id))
        cursor = mydb.cursor()
    ...
    #while True:
        ...
        if '門市業績報表' in msg:
            ...
            try:
                ...
                cursor=mydb.cursor()
                sql="select checked from users where UID=\"%s\""
                data=(str(event.source.user_id))
                cursor = mydb.cursor()
                cursor.execute(sql,data)
